...
class ControllerKlarnaShipping extends Controller {
	...
	public function index() {
		...
		$this->session->data['kco_email']		= $email;
		$this->session->data['kco_postcode']	= $postcode;

		$address = array(
			'postcode'		=> $postcode,
			'country_id'	=> isset($this->session->data['kco_country_id']) 	? $this->session->data['kco_country_id'] 	: $this->config->get('config_country_id'),
			'zone_id'		=> isset($this->session->data['kco_zone_id']) 		? $this->session->data['kco_zone_id'] 		: $this->config->get('config_zone_id'),
			'iso_code_2'	=> isset($this->session->data['kco_country'])		? $this->session->data['kco_country']		: strtoupper($this->language->get('code')),
		);
		...
		foreach ($methods as $key => $value) { $sort_order[$key] = $value['sort_order']; }
